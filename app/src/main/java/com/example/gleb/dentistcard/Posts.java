    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                View dialogView = LayoutInflater.from(Posts.this).inflate(R.layout.update_post, null);
                builder.setView(dialogView);
                AlertDialog dialog = builder.create();
                dialog.show();

                oldPostEditText = (EditText) dialog.findViewById(R.id.oldPostEditText);
                oldPostEditText.append(arrayPosts[position - 1]);
                newPostEditText = (EditText) dialog.findViewById(R.id.newPostEditText);
            ...
            if (item.getItemId() == R.id.item_add) {
                ...
                if (item.getItemId() == R.id.item_delete) {
                    for (int i = 0; i < particients.size(); i++) {
                        Log.d(TAG, arrayPosts[particients.get(i) - 1]);
